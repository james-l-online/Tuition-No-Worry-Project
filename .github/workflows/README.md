...
This folder contains GitHub Actions workflow examples for CI, build, and security checks. Review and
adapt these workflows before enabling them in your project.
...
Files in this directory
- `acr-admin-build-push.yml` — Example: build and push a Docker image to ACR (uses admin creds in the demo).
- `ci-deploy-acr-aks.yml` — Example: build, push, and deploy to AKS (illustrates OIDC and secret usage).
- `secret-scan.yml` — Enabled: runs a repository secret scan (gitleaks) on pushes and PRs.

How to use these workflows

1. Review and adapt the workflow YAML before enabling it. Pay attention to:
  - Secrets and credentials (do NOT commit them).
  - Image names, tags, and Helm/kubectl commands.
  - Branch filters and triggers.
...
- Limit credential scope (use scoped tokens or OIDC where possible).
